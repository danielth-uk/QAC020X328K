...
def ticketBodyNormalize(data: dict, direction: bool) -> dict:
    ...
    if "ops" in data:
        ...
        return HTTPException(
            status_code=500, detail="Dictionary not in required format"
        )
...
def checkAuthentication(username: str, password: str) -> list:
    ...
    if len(result) == 0 or len(result) > 1:
        ...
        if result[0]["admin"]:
            ...
            databaseExecute(
                "UPDATE tbl_users SET token=%s WHERE userid = %s",
                [adminJWT, result[0]["org"] + "." + result[0]["username"]],
            )
            return HTTPException(
                status_code=200,
            ...
            databaseExecute(
                "UPDATE tbl_users SET token=%s WHERE userid = %s",
                [userJWT, result[0]["org"] + "." + result[0]["username"]],
            )
            return HTTPException(
                status_code=200,
                detail="Authenticated",
...
def registerUser(
    username: str, password: str, org: str, name: str, adminCode: str
) -> HTTPException:
    admin = False
    if adminCode != "":
    ...
    try:
        ...
        return HTTPException(
            ...
            headers={"success": True, "reason": "User Created"},
        )
...
def createUser(userDetails) -> HTTPException:
    ...
    if len(exists) > 0:
        raise HTTPException(
            status_code=500,
            detail="Username and org combination already exists",
            headers={"success": False, "reason": "userid already exists"},
        )
    else:
        ...
        return HTTPException(
            status_code=201,
            detail="User Successfully created",
            headers={"success": True, "password": password},
        )
...
def updateUser(details: str, userId: str) -> None:
    ...
    if len(exists) > 0:
        ...
        databaseExecute(
            "UPDATE tbl_users SET userid=%s, username=%s,name=%s WHERE userid = %s",
            [
                details.org + "." + details.username,
                details.username,
                details.name,
...
def adminGetTickets():
    return databaseFetch(
        "SELECT t.*, u1.name AS created_name, u2.name AS assigned_name FROM tbl_tickets AS t INNER JOIN tbl_users AS u1 ON t.created_by = u1.userid LEFT JOIN tbl_users AS u2 ON t.assigned_contact = u2.userid ORDER BY t.opened_time DESC"
    )
...
def adminUpdateAssigned(ticket, data):
    databaseExecute(
        "UPDATE `tbl_tickets` SET `assigned_contact`=%s WHERE id = %s",
        [data.userid, ticket],
    )
    ...
    try:
        if query.query == "RESET":
            ...
            return HTTPException(
                status_code=200,
                detail="Success",
                headers={"status_code": 200, "data": results},
            )
        else:
            databaseExecute(query.query)
            return HTTPException(
                status_code=204,
                detail="No return content",
                headers={"status_code": 204},
            )
    except Exception as e:
        return HTTPException(
            status_code=500,
            detail="Internal Server Error",
            headers={"status_code": 500, "error": e},
        )
...
def clientCreateTicket(username, org, subject, body) -> HTTPException:
    databaseExecute(
        "INSERT INTO `tbl_tickets`(`subject`, `created_by`,`org`, `main_body`) VALUES (%s,%s,%s,%s)",
        [subject, username, org, json.dumps(ticketBodyNormalize(body, True))],
    )
...
def ticketOpenTicket(org: str, ticketId: int) -> dict:
    ...
    if len(data) == 1:
        data[0]["main_body"] = ticketBodyNormalize(
            {"ops": json.loads(data[0]["main_body"])}, False
        )
        return data[0]
    else:
    ...
    if len(data) == 1:
        ...
        for comment in data:
            comment["comment_body"] = ticketBodyNormalize(
                {"ops": json.loads(comment["comment_body"])}, False
            )
        return data
    else:
        raise HTTPException(status_code=404, detail="Ticket Not Found")
...
def ticketCreateTicketComment(org, username, data):
    checkData = databaseFetch(
        "SELECT id from tbl_tickets WHERE id = '%s'" % data.ticket
    )
    if len(checkData) == 1:
        if data.username == username and data.org == org:
            databaseExecute(
                "INSERT INTO `tbl_ticket_comments`(`ticket_id`, `comment_body`, `posted_by`) VALUES (%s,%s,%s)",
                [
                    data.ticket,
                    json.dumps(ticketBodyNormalize(data.body, True)),
                    data.username,
                ],
            )
...
def ticketUpdateComment(data, commentId):
    databaseExecute(
        "UPDATE `tbl_ticket_comments` SET `comment_body`=%s,`updated`= 1 WHERE id=%s",
        [json.dumps(ticketBodyNormalize(data.body, False)), commentId],
    )
